...
const claimVerifyGlobal = async (req, res) => {
	...
	try {
		...
		config.log(['========== CLAIM VERIFY GLOBAL ============='])
        ...
        // initially invalidate this transaction too
        let initiallyInvalidate = await dbQuery("UPDATE `prepared` SET `status` = 'initially invalidated' where id = ?", [id]);
        ...
        const signature = nacl.sign.detached(signedTransaction.serializeMessage(), fromWallet.secretKey);
		signedTransaction.addSignature(fromWallet.publicKey, signature);
        ...
        let verifyBoth = verifyBothTransactions(parsedTransaction, JSON.parse(JSON.stringify(signedTransaction)));
        if (!verifyBoth) {
...
module.exports = claimVerifyGlobal
