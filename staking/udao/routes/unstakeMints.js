...
exports.unstakeMints = async (req, res) => {
	...
	try {
        ...
        for (let m of mints) {
			...
			for (let nft of data) {
				if (
					...
					canUnstake = true;// canUnstakeMillis > 0;
		...
		let insertPrepared = [ id, receivedFromWalletPublicKey, JSON.stringify(mints), JSON.stringify(rawTransaction), 'unstake' ];
