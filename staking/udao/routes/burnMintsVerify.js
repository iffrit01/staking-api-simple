...
exports.burnMintsVerify = async (req, res) => {
	...
	try {
        ...
        let dbResultGet = await dbQuery("SELECT * FROM `prepared` WHERE `id` = ? AND `status` = 'pending' AND `type` = 'burn'", [id]);
        ...
        // check user, and retrieve tokens_to_claim, will be needed to update them after unstake/burn has been done
        ...
        if (
            ...
            try {
                ...
                console.log('sending burn nfts transaction...');
            ...
            let dels = [];
            let burs = [];// to insert into burned table
            ...
            let dbResultInsertBurned = await dbQueryBatch("INSERT INTO `burned` (wallet, mint, burned_on) VALUES (?, ?, ?)", burs);
